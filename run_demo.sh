...
(cd remote_project && \
    DATABASE_NAME=db_remote.sqlite3 ./manage.py migrate --noinput && \
    DATABASE_NAME=db_remote.sqlite3 ./manage.py shell -c "from remote_project.remoteapp.models import Book; Book.objects.create(title='Remote Book 1');" && \
    DATABASE_NAME=db_remote.sqlite3 ./manage.py runserver 8001 &)
REMOTE_PID=$!
...
(cd example_project && \
    ...
    DATABASE_NAME=db_example.sqlite3 ./manage.py sync_local_books && \
